...
## 2026-07-11 — Final tuning and public release

- **Configuration**:
  - Replaced coarse detail presets with an editable 8-48 px character-height control.
  - Added adaptive color depth from 4-64 colors and cached source-cell palette indices.
  - Added a 5-60 FPS selector with a 24 FPS default; media is throttled to the selected rate and displacement physics remains capped at 30 FPS.
  - Added reset-to-defaults and optional custom procedural-color controls.

- **Procedural fixes**:
  - Corrected fire orientation and removed Mandelbrot.
  - Reworked Matrix rain into persistent per-cell state: falling heads deposit stationary characters, selected cells cycle independently, brightness decays by age, and expired cells clear without accumulation.
  - Preserved curated procedural palettes by default while allowing a selected-color override.

- **Performance fixes**:
  - Stopped procedural clock updates from rebuilding static image geometry.
  - Cached adaptive palette assignments so ripple updates no longer perform nearest-color searches for every cell.
  - Kept scene-graph geometry split into backend-safe batches of at most 60,000 vertices.

- **Publication**:
  - Added `README.md`, MIT `LICENSE`, `.gitignore`, and safe build/install/uninstall scripts.
  - Excluded native binaries and build output because they are architecture- and Qt-ABI-specific.
  - Published the source repository at https://github.com/LilT59/ascii-reactive-wallpaper.

- **Validation**:
  - Native build, `ldd -r`, `qmllint`, `xmllint`, metadata validation, and shell syntax checks pass.

---

## 2026-07-11 — System Settings crash fix, multimedia removal, config cleanup

- **Summary**: Diagnosed and fixed a SIGABRT crash when applying the wallpaper from System Settings → Wallpaper. The crash was systemsettings-specific (right-click → Configure Desktop worked fine). Root cause traced through isolation testing to `ImagePath` with KConfigXT `type="Url"` and empty default, which triggers a `QUrl("")` assertion. Fixed by changing to `type="String"` (matching built-in KDE wallpapers). Also removed Qt Multimedia dependency, stripped video/GIF support, and added null-guard protections.

- **Crash diagnosis process**:
  - Isolated the crash to `contents/config/main.xml` by binary-search stripping files from the plugin directory
  - Traced the specific entry to `ImagePath` with `type="Url"` and `<default></default>`
  - Side-discoveries during testing: `QPointF` metatype registration needed for `PointerTracker` singleton (added `qRegisterMetaType<QPointF>()`), null `window()` guard needed in `updatePaintNode()`
  - Qt Multimedia (`Qt6::Multimedia`) linkage was removed entirely — systemsettings doesn't have the multimedia backend, and pulling it in at .so load time could also cause issues

- **Key changes**:
  - `contents/config/main.xml`: `ImagePath` type `Url` → `String` (the actual crash fix)
  - `contents/config/main.xml`: Removed obsolete `Detail` entry
  - `native/CMakeLists.txt`: Removed `Qt6::Multimedia` from `find_package` and `target_link_libraries`
  - `native/asciirenderer.cpp`: Removed all `QMediaPlayer`/`QMovie`/`QVideoSink` code; added null `window()` guard in `updatePaintNode()`; removed media throttle in `processImage()`
  - `native/asciirenderer.h`: Removed multimedia member variables (`m_movie`, `m_player`, `m_videoSink`, `m_mediaThrottle`)
  - `native/pointerplugin.cpp`: Added `qRegisterMetaType<QPointF>()` before singleton registration
  - `native/pointertracker.h`: Changed `pressed(QPointF, int)` signal to `pressed(qreal x, qreal y, int button)`
  - `native/pointertracker.cpp`: Updated `emit pressed(...)` to use `.x()` and `.y()`; restored polling timer
  - `contents/ui/main.qml`: Added `!renderer` guards on `Connections` handlers; updated `onPressed` to use `(x, y, button)` signature
  - `contents/ui/config.qml`: Removed video references ("Image or video" → "Image"); changed FileDialog filter to images only

- **Decisions**:
  - Video/GIF wallpaper support removed rather than implementing `QLibrary` runtime loading — user doesn't care about videos
  - Static image-to-ASCII still works via `QImage` (no multimedia needed)
  - Aspect-ratio correction for oval click ripples attempted but reverted — the Laplacian-weighted wave equation was unstable with current tension/damping values. Known issue, not a priority.
  - `QPointF` kept in `Q_PROPERTY` and `globalPosition()` getter (only removed from signal params)

- **Results**:
  - System Settings → Apply now works without crash
  - Right-click → Configure Desktop still works
  - All 6 procedural modes + pointer displacement + click ripples functional
  - Static image source still supported
  - Config UI cleaned up (no video references)

- **Next steps**:
  - Fix oval click ripple shape (needs stable anisotropic wave equation or alternative approach)
  - Consider re-adding video/GIF via `QLibrary` runtime loading if needed
  - Profile and polish as listed in AGENTS.md remaining work
  - Commit and push changes to GitHub

---

## 2026-07-11 — Demo GIF and oval ripple investigation

- **Summary**: Created a demo GIF for the README (1000px, 256-color Floyd-Steinberg dither, gamma 1.3). Investigated the oval click-ripple shape — root cause is the non-square character grid (11×19px cells) with an isotropic discrete Laplacian wave solver. The wave propagates ~1.7× faster vertically in pixel space because vertical cells are taller. Attempted aspect-ratio corrections in `addImpulse()`, `stepSimulation()`, and `regenerateCharacters()` — all either ineffective or destabilized the simulation. The existing tension/damping values (0.18/0.92) are tuned for the isotropic solver and can't accommodate Laplacian weighting. Fixed requires a full solver rewrite with anisotropic coefficients.

- **Key changes**:
  - Recorded screen demo via Kooha, converted to README GIF with ffmpeg
  - Updated AGENTS.md quality checklist — all items verified working except QML warnings
  - Attempted and reverted multiple ripple shape fixes (see above)

- **Known issues**:
  - Click ripples appear oval (taller than wide) due to non-square character cell aspect ratio
  - Some QML warnings from Plasma's own KCM wallpaper module (not our plugin)
